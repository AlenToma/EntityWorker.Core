...
using EntityWorker.Core.Postgres.Logging;
...
#pragma warning restore CA1801 // Review unused parameters
        ...
        internal void TryRemovePendingEnlistedConnector(NpgsqlConnector connector, System.Transactions.Transaction transaction)
        {
            lock (_pendingEnlistedConnectors)
            {
                if (!_pendingEnlistedConnectors.TryGetValue(transaction, out var list))
        ...
        [CanBeNull]
        internal NpgsqlConnector TryAllocateEnlistedPending(System.Transactions.Transaction transaction)
        {
            lock (_pendingEnlistedConnectors)
            {
                if (!_pendingEnlistedConnectors.TryGetValue(transaction, out var list))
        ...
        readonly Dictionary<System.Transactions.Transaction, List<NpgsqlConnector>> _pendingEnlistedConnectors
            = new Dictionary<System.Transactions.Transaction, List<NpgsqlConnector>>();
