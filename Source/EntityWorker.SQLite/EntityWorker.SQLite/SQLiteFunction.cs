...
namespace EntityWorker.SQLite
{
	public abstract class SQLiteFunction : IDisposable
	{
		...
		public EntityWorker.SQLite.SQLiteConvert SQLiteConvert
		{
			get
			{
				this.CheckDisposed();
		...
		internal int CompareCallback(IntPtr ptr, int len1, IntPtr ptr1, int len2, IntPtr ptr2)
		{
			...
			try
			{
				num = this.Compare(EntityWorker.SQLite.SQLiteConvert.UTF8ToString(ptr1, len1), EntityWorker.SQLite.SQLiteConvert.UTF8ToString(ptr2, len2));
			}
		...
		private void SetReturnValue(IntPtr context, object returnValue)
		{
			...
			switch (EntityWorker.SQLite.SQLiteConvert.TypeToAffinity(type, this._flags))
			{
